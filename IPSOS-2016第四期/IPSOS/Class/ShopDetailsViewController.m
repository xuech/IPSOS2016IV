...
@interface ShopDetailsViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,FTPManagerDelegate,UIActionSheetDelegate,DataManagerDelegate,MKMapViewDelegate,CLLocationManagerDelegate>
{
    BOOL isFirst;
    StoreData *storeData;
...
@property (weak, nonatomic) IBOutlet UIImageView *readYes;

@property (weak, nonatomic) IBOutlet UIButton *completeButton;
@property (weak, nonatomic) IBOutlet UIView *otherView;//被访者原话记录
...
//    [_completeButton setAlpha:1.0f];
    ...
    if ([[plistData valueForKey:@"IsRead"] boolValue]) {
        [_readYes setHidden:NO];
    }else{
        [_readYes setHidden:YES];
    }
...
//    }
    ...
    //如果都选择以上皆无选项时隐藏拍照
    [_takePhotoView setHidden:NO];
    [_questionnaireView setHidden:NO];
    
    //2016 第三期修改
    BOOL showQuestionnaireView = true;
    NSArray* taskData2 = [plistData valueForKey:@"taskData2"];
    NSUInteger count1 = 0;
...
//        if (i<=4) {
            ...
            if (result.count > 0) {
                ...
                if (count1 == taskData2.count) {
                    [_questionnaireView setHidden:YES];
                }
//                showQuestionnaireView = count1 == taskData2.count?false:true;
...
//                NSLog(@"%@",re);
//            }
//            if (([result containsObject:@"糖尿病健康知识宝典-陈列架"]|| [result containsObject:@"糖尿病心天地APP教育单页–陈列架"])&&([result containsObject:@"糖尿病健康知识宝典-患教书籍(茶杯)"]|| [result containsObject:@"糖尿病健康知识宝典-患教书籍(无茶杯)"]||[result containsObject:@"糖尿病心天地APP教育单页–患教书籍"])) {
//                selectedAll = true;
//            }
//        }
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    [_completeButton setAlpha:0.0f];
    self.title = @"";
    if ([_questionnaireView isHidden]) {
        [self changePhotoViewLocationFirst:NO];
    }
}
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    //
    if (![_signInYes isHidden]&&![_shopTask2Yes isHidden]&&![_questionYes isHidden]) {
        if (![_takePhotoView isHidden]&&![_photoYes isHidden]) {
            if ([_otherView isHidden]&&![_questionnaireView isHidden]) {
    ...
    if (![_signInYes isHidden]&&![_shopTask2Yes isHidden]) {
        if ([_questionnaireView isHidden]) {
            [self changePhotoViewLocationFirst:(BOOL)YES];
        }
    }
...
-(void)changePhotoViewLocationFirst:(BOOL)first
{
    CGRect frame = [_completeButton frame];
    CGRect frameRecord = [_takePhotoView frame];
    if (first) {
        frame.origin.y -= [_questionnaireView frame].size.height;
        frameRecord.origin.y -= [_questionnaireView frame].size.height;
    }else{
        frame.origin.y += [_questionnaireView frame].size.height;
        frameRecord.origin.y += [_questionnaireView frame].size.height;
    }
    [_takePhotoView setFrame:frameRecord];
...
//        return;
//    }
    [self performSegueWithIdentifier:@"shopTaskTable2" sender:nil];
    [Flurry logEvent:@"观察记录（促销陈列形式）" withParameters:[Context getSystemInfo]];
}
- (IBAction)readyTaskEvent:(UIButton *)sender {
    if (![_readYes isHidden]) {
        return;
    }
    if ([_signInYes isHidden]) {
        [ZAActivityBar showErrorWithStatus:@"请先签到"];
    ...
    [self performSegueWithIdentifier:@"readyTask" sender:nil];
    [Flurry logEvent:@"出示" withParameters:[Context getSystemInfo]];
}
...
//    }
    ...
    if ([_readYes isHidden]) {
        [ZAActivityBar showErrorWithStatus:@"请先阅读出示"];
        return;
    }
...
-(void)uploadZip
{
    ...
    //|| [_questionYes isHidden] || [_recordYes isHidden]|| [_shopTaskYes isHidden]|| [_questionYes isHidden]
    if ([_signInYes isHidden]   || [_shopTask2Yes isHidden]) {
